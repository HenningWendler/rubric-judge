...
def test_compare_serves_the_comparison_of_two_runs(client):
    ...
    assert body["case_comparisons"][0]["criterion_comparisons"][0]["status"] == "improved"
...
def test_the_comparison_carries_every_published_field(client):
    ...
    assert set(body) == {"metrics_delta", "summary", "case_comparisons"}
    ...
    assert set(body["case_comparisons"][0]) == {
        "case_id", "baseline_score", "candidate_score", "score_delta", "status",
        "criterion_comparisons",
    }
    assert set(body["case_comparisons"][0]["criterion_comparisons"][0]) == {
